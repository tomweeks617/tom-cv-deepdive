...
- ~~Question logging~~ — **done 2026-06-12** via structured `console.log` in the chat route (`chat_question` + `chat_answer` events, hashed visitor IDs, token/cache usage). Still no datastore — decision 8 stands; retention requires a Vercel log drain (Axiom), which is a manual connect for Tom.
